...
Requires a maintained Node build with patched SQLite (known baselines: Node 22.22.3 or 24.15.0), systemd 249+ and Apache 2.4.47+ for these historical inbound templates. See [runtime safety](../../docs/sqlite-runtime-safety.md); the current outbound-pull service does not require Apache. A Node installation under an administrator's home is not the service runtime. Install a separately verified Linux/architecture-matching runtime under `/usr/local/lib/oaf-wake/runtime`, without replacing the system Node or relying on nvm/profile startup. All ancestors and release/runtime directories must be root-owned and not group/world-writable. Keep the actual Node/SQLite versions and upstream checksum in the private deployment record.
...
Build from a clean, reviewed commit with patched Node SQLite and the project's pnpm 10.30.3. Do not build as root on the shared production host or install packages at service startup.
...
The helper creates a **local directory**, not a remote deployment. The current runtime has exactly two packages: wake-service and protocol, with no external runtime dependencies. The helper copies their built output and READMEs, removes development metadata and pins the included protocol version. It refuses an existing output directory or a changed dependency graph instead of silently omitting new dependencies. It does not resolve newer dependencies from the registry or copy workspace symlinks. The checker verifies package contents, link containment, imports and the actual SQLite engine using an in-memory probe, without opening service databases, starting a listener or sending a callback. Test the artifact with the target Linux Node binary before installation; a successful macOS check is not proof of Linux service/proxy activation. Refusal requires a reviewed runtime upgrade, never deletion of either durable database.

Archive the complete `app` directory, preserving its relative dependency symlinks. Exclude host filesystem metadata: on macOS, ordinary tar can add AppleDouble `._*` entries that the Linux artifact checker correctly rejects. Use `COPYFILE_DISABLE=1 tar --no-xattrs -czf wake-app.tar.gz -C <release-parent> app`, then verify the extracted artifact on the target runtime. Record the reviewed commit and artifact SHA-256 privately. On the approved host, unpack into a new root-owned immutable `/usr/local/lib/oaf-wake/releases/<commit>` directory and verify the transferred checksum. Keep `/usr/local/lib/oaf-wake/current` a root-managed symlink to that release. A symlink for **code** is intentional; the token, database and final state directory must not be symlinks.
