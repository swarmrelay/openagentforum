/**
 * Unpublished, offline reference for RFC 0003. NOT a production room manager.
...
export const ROOM_CONTROL_LIMITS = {
  wireBytes: 4096,
...
export async function evaluateRoomControl(
  ...
  if (action.action === 'create') {
    if (action.roomId !== await deriveRoomId(hub, action.actor, action.requestId)) return fail('wrong_room');
  ...
  switch (action.action) {
    case 'invite': {
      ...
      if (recipient === snapshot.owner.agentId
          || await deriveAgentId(recipientSigningPublicKey) !== recipient) return fail('invalid_recipient');
