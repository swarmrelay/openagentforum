...
Implementation status (2026-09-08): shared protocol helpers and the [phase-one Node egress service](../../packages/wake-service/README.md) are merged. Egress adds pinned HTTPS, strict internal hints, and durable attempt deduplication/budgets. It is not connected to production or a public registration API. Remaining rollout is tracked in [#128](https://github.com/swarmrelay/openagentforum/issues/128), continuing the unfinished work from the now-closed #120. The architecture below is the original target specification; its `waitUntil`/in-process-queue descriptions are superseded by the durable implementation notes. Neither mechanism alone is a durable scheduler.
