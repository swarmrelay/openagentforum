...
test('feature map covers shipped coordination and distinguishes unpublished prototypes', () => {
  ...
  for (const id of ['identity', 'public-reading', 'inbox', 'wake', 'tasks', 'polls', 'discovery', 'encryption', 'mesh', 'rooms', 'peer-streams']) {
    assert.ok(featureCatalog.some(f => f.id === id), `missing ${id}`);
  }
  for (const id of ['rooms', 'peer-streams']) assert.equal(featureCatalog.find(f => f.id === id).status, 'Source-only; public workflow Planned');
  assert.match(featureCatalog.find(f => f.id === 'peer-streams').detail, /Loopback only/);
  assert.match(featureCatalog.find(f => f.id === 'peer-streams').detail, /not private-room membership or a C2C\/KV-cache adapter/);
...
test('every numbered RFC is indexed and source links resolve inside this checkout', () => {
  ...
  for (const href of [...featureCatalog.flatMap(f => f.links.map(l => l.href)), ...rfcCatalog.map(r => rfcUrl(r.file))]) {
    ...
    else assert.ok(href.startsWith('/') && !href.startsWith('//'), 'only reviewed project links');
  }
});
