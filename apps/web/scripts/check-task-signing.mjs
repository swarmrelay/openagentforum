...
export function validateTaskSigning(files) {
  const errors = [];
  const page = nodes(parse(String(files.get('tasks/index.html') ?? '')));
  const sections = page.filter(n => attr(n, 'id') === 'task-signing');
  if (sections.length !== 1) errors.push('tasks/index.html: expected one task-signing section');
  ...
  for (const value of [taskSigningTitle, ...taskSigningParagraphs, taskSigningFailures, taskClaimExampleIntro, taskClaimExampleBoundary, taskSigningPaymentBoundary]) {
    if (!visible.includes(value)) errors.push(`tasks/index.html: task guidance differs: ${value}`);
  }
  for (const value of [taskSigningProof, taskClaimExample]) {
    if (!contents.some(n => n.tagName === 'code' && text(n) === value)) errors.push('tasks/index.html: task proof or claim example differs');
  }
  for (const action of taskSigningActions) {
    ...
    if (rows.length !== 1 || ![action.label, action.detail].every(value => text(row).includes(value))) errors.push(`tasks/index.html: task action differs: ${action.action}`);
    for (const value of [action.route, action.payload, action.body]) {
      if (!nodes(row).some(n => n.tagName === 'code' && text(n) === value)) errors.push(`tasks/index.html: task action contract differs: ${action.action}`);
    }
  }
  for (const href of [taskSigningReference, '/payments/']) {
    if (!contents.some(n => n.tagName === 'a' && attr(n, 'href') === href)) errors.push(`tasks/index.html: missing task reference: ${href}`);
  }
  ...
  for (const file of ['tasks/index.html', 'agent.md', 'llms-full.txt']) {
