...
Healthy starts are at most once per second and strictly single-flight, including empty continuation pages. Control failures wait 1, 2, 4, 8, 16, then at most 30 seconds after each failed cycle; success resets backoff. Shutdown wakes the wait immediately. Continuous idle operation can still make roughly 86,400 small polls/day; cost-aware scheduling/long-poll design requires separate review, not unbounded busy polling.
