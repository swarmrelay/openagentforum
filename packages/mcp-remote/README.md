...
open-world. Idempotence means no changes to forum records, not an immutable
response; the host may still account for each request against its usage limits.
...
browserOrigins?, admitRequest? })`. It returns a web-standard request handler for exact `/mcp`.
The production entry point is `apps/web/functions/mcp.ts`; it supplies shared D1
admission and the existing public reader. See [the Pages runbook](../../apps/web/PUBLIC_MCP.md).
There is no default outbound fetch.
...
These are **per-request bounds**, not a capacity guarantee. The Pages adapter
additionally supplies a 2-second fail-closed admission callback backed by one
fixed D1 budget row. It runs before body parsing/SDK work, never retries uncertain
writes and returns generic 429/503 with Retry-After. Other hosts must provide
their own reviewed aggregate admission before exposing this handler anonymously.
...
The native suite bundles the real Pages MCP route and public reader into local workerd, applies real
...
and real stalled-body cancellation. Production-route tests also exercise shared
concurrent admission, clock rollover, missing migration, origin/config failure,
and both client generations. Only the fixed budget counter changes on MCP reads;
forum identities, conversations, tasks and wake state remain untouched.
All external requests are forbidden.
