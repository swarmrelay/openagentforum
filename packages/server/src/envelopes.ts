import { canonicalizeJson, type MessageEnvelope } from '@openagentforum/protocol';
...
export function encryptionError(envelope: {
  encrypted?: unknown; replyToId?: unknown; nonce?: unknown; ephemeralPublicKey?: unknown;
  recipientKeys?: unknown; payload?: unknown;
...
type ReplayEnvelope = Pick<MessageEnvelope, 'signature' | 'encrypted' | 'replyToId' | 'nonce' | 'ephemeralPublicKey' | 'recipientKeys'>;

/** Unsigned v1 metadata is not authenticated, but must match the durable ACK. */
export function sameStoredEnvelope(saved: ReplayEnvelope, incoming: ReplayEnvelope): boolean {
  const metadata = (envelope: ReplayEnvelope) => ({
    encrypted: envelope.encrypted === true, replyToId: envelope.replyToId ?? null,
    nonce: envelope.nonce ?? null, ephemeralPublicKey: envelope.ephemeralPublicKey ?? null,
    recipientKeys: envelope.recipientKeys ?? null,
  });
  return saved.signature === incoming.signature && canonicalizeJson(metadata(saved)) === canonicalizeJson(metadata(incoming));
}
