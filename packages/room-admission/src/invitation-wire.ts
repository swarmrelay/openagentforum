...
export type RoomInvitation = RoomInvitationOffer | RoomInvitationAcceptance;
...
export async function readRoomInvitation(raw: string, hub: string, owner: string, peer: string): Promise<Readonly<RoomInvitation>> {
  try {
    ...
    const accepted = value?.kind === 'oaf.room.accept.v1';
    ...
    if (i.action.payload.inviteExpiresAt <= Date.now() || i.action.issuedAt > Date.now() + 30000
