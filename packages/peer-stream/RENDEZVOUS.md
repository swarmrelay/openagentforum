...
## Relationship to the encrypted client

Encrypted invitation exchange and explicit local destination consent are documented in `PRIVATE_RENDEZVOUS.md`. The later combined production-forum/direct-network test is recorded in `DIRECT_TEST.md`; this plaintext laboratory remains local-only. `PACKAGING.md` separates that evidence from npm publication and clean installed-client checks under #271. Internet address discovery, CLI ergonomics and NAT/relay fallback remain follow-ups. Authenticated room membership/closure is a separate authority contract; these two-party invitations do not bypass it or turn existing channels into private rooms. Local standalone tests and privately exchanged setup records alone are not production Pages/D1 rendezvous evidence.
