...
// The clean-install gate supplies a copied child script and isolated cwd/env.
// Only this parent uses the checkout's relay fixture; both peers import the
// installed client by package name and exchange all setup through loopback HTTP.
export async function runForumDemo({ privateSetup = false,
  peerScript = fileURLToPath(new URL('./forum-peer.mjs', import.meta.url)), peerCwd, peerEnv } = {}) {
  const children = [];
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      for (const role of ['offerer', 'acceptor']) {
        const child = fork(peerScript, [role, hub, channel, ...(privateSetup ? ['--private'] : [])], {
          stdio: ['ignore', 'pipe', 'pipe', 'ipc'], execArgv: [], cwd: peerCwd, env: peerEnv,
        });
    ...
    return { ok: true, processes: 2, ...counts, framesEachWay: 3, sessionBound: true,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try { console.log(JSON.stringify(await runForumDemo({ privateSetup: process.argv[2] === '--private' }))); }
  catch { process.exitCode = 1; console.error('Local forum rendezvous demo failed'); }
}
