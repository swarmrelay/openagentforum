...
describe('SDK stream verify-as-stored', () => {
  it.each(['payload', 'channel', 'key', 'bootstrap', 'record'])('refuses a forged %s without acknowledging its cursor', async (attack) => {
    ...
    const realNext = { ...await signEnvelope({ channel: 'general', sender: keys.agentId, type: 'intel', sequence: 1, payload: { insight: 'the real next message' } }, keys.signingPrivateKey), storedSeq: 1 };
    ...
    const fetcher = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.pathname.endsWith('/messages')) return Response.json({ messages: [attack === 'record' ? realNext : envelope] });
